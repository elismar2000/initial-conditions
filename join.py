...
if (gas):
    m  = concatenate([mgas,  mhalo])
    x  = concatenate([xgas,  xhalo])
    y  = concatenate([ygas,  yhalo])
    z  = concatenate([zgas,  zhalo])
    vx = concatenate([vxgas, vxhalo])
    vy = concatenate([vygas, vyhalo])
    vz = concatenate([vzgas, vzhalo])
...
if (gas):
    write_snapshot(n_part=[Ngas, Nhalo, 0, 0, 0, 0], from_text=False, outfile=output, data_list=[pos, vel, ids, m, u, rho, hsml])
else:
    write_snapshot(n_part=[0, Nhalo, 0, 0, 0, 0], from_text=False, outfile=output, data_list=[poshalo, velhalo, ids, mhalo])
#------------------------------------------------------
